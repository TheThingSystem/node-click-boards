/* this fails to write, letting me to believe that _ACC_I2C_ADDR is wrong
 */
...
var compass = function(options) {
  ...
  if (!self.options.address) self.options.accel_address = 0x32;
  if (!self.options.address) self.options.magno_address = 0x3c;
...
compass.prototype._steadyAcceleration = function(callback) {
  ...
  self._averageMeasurement(self.measureAcceleration, 8, function(err, result) {
    ...
    callback(null, helpers.v_magnitude(result) < 1250);
  });
};
